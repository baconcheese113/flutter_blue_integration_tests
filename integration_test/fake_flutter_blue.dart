...
class FakeBluetoothCharacteristic implements BluetoothCharacteristic {
  ...
  FakeBluetoothCharacteristic.create(Guid uuid)
      : _fakeUuid = uuid,
        _fakeValue = utf8.encode("FirstRead");
  ...
  Future<bool> setNotifyValue(bool notify) async {
    ...
    print("Starting to notify $notify");
  ...
  Future<Null> write(List<int> value, {bool withoutResponse = false}) async {
    ...
    if (utf8.decode(value) == "FirstWrite") _fakeValue = utf8.encode("SecondRead");
    if (utf8.decode(value) == "SecondWrite") _fakeValue = utf8.encode("ThirdRead");
    return null;
  }
...
class FakeBluetoothDevice extends BluetoothDevice {
  ...
  Future<List<BluetoothService>> discoverServices() async {
    ...
    final chars = [
      FakeBluetoothCharacteristic.create(Guid(DEVICE_NAME_CHARACTERISTIC_UUID)),
      FakeBluetoothCharacteristic.create(Guid("00002a01-0000-1000-8000-00805F9B34FB")),
    ];
    final service = FakeBluetoothService.create(Guid(GENERIC_ACCESS_SERVICE_UUID), chars);
